        ...
        for i, tok in enumerate(word_pieces):
            if "##" in tok:
                ...
                try:
                    ...
                    # ignore a sentence and continue with the loop
                    return [], []
...
def data_to_tensors(dataset: List, tokenizer: BertTokenizer, max_len: int, labels: List=None, predicate_sequences: List=None, label2index: Dict=None, pad_token_label_id: int=-100) -> Tuple:
    ...
    for i, sentence in enumerate(dataset):
        ...
        if predicate_sequences:
             pred_seq_indices.append(predicate_sequences[i])
    ...
    if predicate_sequences:
        pred_seq_indices = pad_sequences(pred_seq_indices, maxlen=max_len, dtype="long", value=0, truncating="post", padding="post")
        pred_seq_indices = LongTensor(pred_seq_indices)
 
    else:
        pred_seq_indices = None
    ...
    return LongTensor(input_ids), LongTensor(attention_masks), label_ids, LongTensor(seq_lengths), pred_seq_indices
...
def read_json(filename):
    ...
    with open(filename, 'r') as json_file:
        for line in json_file:
            ...
            seq_words = data['seq_words']
            bio_labels = data['BIO']
            pred_sense = data['pred_sense'] if data['pred_sense'] else [None]

            # create binary sequence to distinguish predicate
            pred_index = pred_sense[0] if isinstance(pred_sense[0], int) else None
            binary_sequence = ['1' if i == pred_index else '0' for i in range(len(seq_words))]

            all_sentences.append(seq_words)
            all_labels.append(bio_labels)
            all_pred_sense.append(binary_sequence)
            label_dict = add_to_label_dict(bio_labels, label_dict)
...
def evaluate_bert_model(eval_dataloader: DataLoader, eval_batch_size: int, model: BertModel, tokenizer:BertTokenizer, label_map: dict, 
    ...
    for batch in eval_dataloader:
        ...
        # I admit this is not an elegent solution but this is a more generalizable code which can work with or without predicate_sequences  
        if len(batch) == 4:
            # Unpack the inputs from our dataloader
            b_input_ids, b_input_mask, b_labels, b_len = batch
        else:
            b_input_ids, b_input_mask, b_labels, b_len, train_pred = batch
